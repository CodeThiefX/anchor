...
export default function LoanTrackerClient() {
  ...
  // Move the default values to state variables
  const [status, setStatus] = useState<LoanStatus>("pending");
  const [loanId, setLoanId] = useState("0001");
  const [rejectionReason, setRejectionReason] = useState(
    "Insufficient income documentation for the requested loan amount."
  );
  const [paymentAmount, setPaymentAmount] = useState(873000);
  const [paymentDueDate, setPaymentDueDate] = useState("March 12th, 2025");
